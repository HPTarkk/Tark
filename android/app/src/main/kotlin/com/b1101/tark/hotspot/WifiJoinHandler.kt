...
class WifiJoinHandler(
    ...
    /**
     * Some OEMs associate with the requested hotspot before the specifier
     * callback reaches the app. Without an independent observation the UI then
     * spins for the whole framework timeout even though a manual "enter room"
     * immediately works. This probe is scoped to the exact pending join and
     * accepts only a readable SSID that exactly matches the requested AP.
     */
    ...
    private fun join(
        ...
        val cb = object : ConnectivityManager.NetworkCallback() {
            override fun onAvailable(network: Network) {
                ...
                Log.i(TAG, "specifier onAvailable after ${SystemClock.elapsedRealtime() - startedAt}ms")
                bind(network)
                joinedSsid = ssid
            ...
            override fun onUnavailable() {
                ...
                // One final evidence check closes the race where association
                // happened at the same moment the framework declared its
                // foreground-scoped request unavailable.
    ...
    /**
     * Accepts current-network evidence only when Android exposes a readable
     * SSID and it exactly matches the AP requested by this pending join. An
     * unknown/redacted SSID is deliberately NOT enough positive evidence here:
     * otherwise a remembered home Wi-Fi could be mistaken for the room link.
     */
    ...
    private fun installSuggestion(ssid: String, passphrase: String, security: String) {
        ...
        if (status == WifiManager.STATUS_NETWORK_SUGGESTIONS_SUCCESS) {
            suggested = list
            Log.i(TAG, "suggestion installed for joined AP")
        } else {
    ...
    private inner class PendingJoin(
        ...
        val isPending: Boolean get() = !replied.get()
