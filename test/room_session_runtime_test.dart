...
void main() {
  ...
  test(
    'transport replacement preserves logical room, members and mute',
    () async {
      final value = runtime();
  ...
  test(
    'replacement disposes old attachment but keeps room resources',
    () async {
      final value = runtime();
      var roomDisposed = 0;
  ...
  test('stale generation cannot register resources or change state', () async {
    ...
    expect(value.ownCurrentAttachmentResource(first, 'stale', () {}), isFalse);
  ...
  test(
    'degraded and recovery transitions do not logically leave room',
    () async {
      final value = runtime();
  ...
  test(
    'explicit leave is terminal and disposes resources exactly once',
    () async {
      final value = runtime();
      var roomDisposed = 0;
      ...
      expect(() => value.setMuted(false), throwsA(isA<StateError>()));
    },
  );
