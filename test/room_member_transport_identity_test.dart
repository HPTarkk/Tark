...
void main() {
  ...
  test('issuer certificate round-trips and verifies only under trusted root', () async {
  ...
  test('matched heartbeat proof is bound to room token and session epoch', () async {
    final value = await identity();
  ...
  test('member cannot sign proof with a key not named by certificate', () async {
    final value = await identity();
    final attacker = await crypto.generateKeyPair();
  ...
  test('key material decoder rejects malformed key sizes', () {
    expect(
      () => RoomMemberTransportKeyPair.decode(
        privateKey: 'AA',
        publicKey: 'AA',
      ),
