...
void main() {
  ...
  test('replacement disposes old health subscription and ignores stale events', () async {
    final runtime = room();
    ...
    expect(runtime.state.attachment.phase, TransportAttachmentPhase.connecting);
  ...
  test('down and stream failure remain transport failures, not room leave', () async {
    final runtime = room();
    final health = StreamController<ConnectionHealth>.broadcast(sync: true);
    final adapter = RoomTransportHealthRuntimeAdapter(runtime);
    ...
    final replacement = StreamController<ConnectionHealth>.broadcast(sync: true);
  ...
  test('explicit leave disposes the current health subscription once', () async {
    final runtime = room();
    var listens = 0;
    var cancels = 0;
