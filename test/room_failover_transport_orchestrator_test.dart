...
void main() {
  ...
  test('starts replacement transport and gates callbacks to its epoch', () async {
    final room = session();
  ...
  test('new failover disposes the previous transport and rejects stale callbacks', () async {
    final room = session();
    final runtime = RoomFailoverRuntime(session: room);
  ...
  test('stale transport completing after a newer election is disposed immediately', () async {
    final room = session();
    final runtime = RoomFailoverRuntime(session: room);
  ...
  test('start failure is reported on the current attachment without leaving room', () async {
    final room = session();
    final runtime = RoomFailoverRuntime(session: room);
    final orchestrator = RoomFailoverTransportOrchestrator(
      runtime: runtime,
  ...
  test('cancel disposes active replacement without leaving durable room', () async {
    final room = session();
    final runtime = RoomFailoverRuntime(session: room);
    var disposed = 0;
    final orchestrator = RoomFailoverTransportOrchestrator(
