...
void main() {
  ...
  SavedRoom savedRoom() => SavedRoom(
    room: Room(
      ...
      members: [
        ...
        RoomMember(id: acceptedId, displayName: 'Rider three', joinedAt: now),
        RoomMember(
  ...
  test('round trips only durable active Room state and timestamps', () {
    ...
    expect(
      decoded.members.first.joinedAt,
  ...
  test('accepted member survives bounded large roster', () {
    final members = <RoomMember>[
      ...
      RoomMember(id: acceptedId, displayName: 'Accepted rider', joinedAt: now),
    ];
