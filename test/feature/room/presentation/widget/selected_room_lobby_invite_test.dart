...
void main() {
  ...
  testWidgets('one confirmed member stays a one-person Room', (tester) async {
    ...
    // Start needs somebody to start with. In a Room of one it could only ever
    // fail with "nobody answered", so inviting is the only action offered.
  ...
  testWidgets('member without invite authority has no dead invite action', (
    ...
    // Alone and unable to invite: no dead Start either, just what has to
    // happen first.
    expect(find.byKey(const Key('selected-room-start-ride')), findsNothing);
    expect(
      find.byKey(const Key('selected-room-alone-no-invite')),
      findsOneWidget,
    );
    expect(tester.takeException(), isNull);
  });
