...
void main() {
  test(
    'attachment replacement disposes only the replaced generation',
    () async {
  ...
  test(
    'explicit leave disposes attachments before room resources exactly once',
    () async {
      final owner = RoomSessionResourceOwner();
      final disposed = <String>[];
  ...
  test('a disposed owner rejects new resources', () async {
    final owner = RoomSessionResourceOwner();
    await owner.disposeAll();
    expect(() => owner.ownRoomResource('late', () {}), throwsStateError);
    expect(
  ...
  test(
    'duplicate ownership keys fail instead of leaking the previous resource',
    () {
      final owner = RoomSessionResourceOwner();
