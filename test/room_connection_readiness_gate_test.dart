...
void main() {
  ...
  test('healthy attachment plus current signed peer proof becomes ready', () async {
  ...
  test('healthy transport without authenticated peer proof stays closed', () async {
    final subject = runtime();
    final generation = await subject.attach(kind: TransportKind.hotspot);
    subject.ready(generation: generation);
    const epoch = 1;

    final result = await const RoomConnectionReadinessGate(
      timeout: Duration(milliseconds: 20),
    ).wait(
      runtime: subject,
    ...
    expect(result.failure, RoomConnectionReadinessFailureStage.peerProofMissing);
    await subject.leave();
  });
