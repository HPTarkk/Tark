...
- **One scan and you're talking** — the host shows an invite, the other phone scans it, and that is the whole setup. The two phones find each other over Bluetooth, the phone that showed the code brings a hotspot up, the other joins it, and both go on air by themselves: nobody presses Start twice, and nobody is asked who hosts. If Android asks the joining phone whether to connect to a network, that is the one tap left. The host's screen says who just joined, and adding someone mid-ride hands them the call's network without rebuilding it. When a scan can't complete — Bluetooth off, the permission refused, the host out of range — the scanner says which, rather than sitting on "joining".
- **…and it doesn't claim more than it knows.** A room only counts as connected once another member has answered a signed challenge over the link the call is actually using — being on Wi-Fi, even the same Wi-Fi, proves nothing on its own. Start isn't offered in a room of one, and before anyone connects, other members' rows say nothing about their phones. When there's no scan to work from — after a restart, say — Start tries whatever link the phone already has; if that doesn't reach anyone, it says so in plain words and offers **Connect phones** right there.
...
The rules, which are pinned by `test/feature/legal/presentation/manager/legal_consent_test.dart`:
