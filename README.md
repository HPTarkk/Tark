...
- **…and it doesn't overstate what it found.** Being on Wi-Fi is not being on *their* Wi-Fi, and nothing a phone can check before someone is actually heard tells the two apart — matching network names least of all, since that is exactly what two phones look like on one café network with client isolation switched on. So only Bluetooth, where the link *is* the other person, is reported as connected. A network or a hotspot of your own is reported as what it is, with the gap named and one tap to close it: **Not on the same network?** The channel carries the same way out — if the room has people in it and none of them can be heard, it stops offering an invite code (they're already in) and offers to get the phones onto one network instead.
