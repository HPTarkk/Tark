...
- **A room never opens onto nothing** — pressing Start ride used to work whether or not the phone was on anything, and a channel with one person in it reads as a broken app rather than as a phone that isn't connected. The room now checks first — on a Wi-Fi network, holding a hotspot up, joined to someone else's, or paired over Bluetooth, any one will do — and says which. With none of them it doesn't refuse: it leads with the one thing that fixes it, on the side the room already implies (whoever hands out the invite code is the one who makes the network). Coming back from that screen goes straight on air rather than asking again.
- **…and it doesn't overstate what it found.** Being on Wi-Fi is not being on *their* Wi-Fi, and nothing a phone can check before someone is actually heard tells the two apart — matching network names least of all, since that is exactly what two phones look like on one café network with client isolation switched on. So only Bluetooth, where the link *is* the other person, is reported as connected. Everything else is reported as what it is, and the room stops arguing for it: on a network nobody arranged — the home Wi-Fi you're about to walk out of the door with — **Get on one network** is the button, and *we're already on the same network — start* is the quiet line under it. Both routes are one tap; what changed is which one the app stands behind. A hotspot you raised, or one you joined by scanning a code, is an arrangement rather than a guess, so it keeps Start ride. The channel carries the same way out — if the room has people in it and none of them can be heard, it stops offering an invite code (they're already in) and offers to get the phones onto one network instead.
