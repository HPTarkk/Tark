...
- **VOX (voice-activated)** — no button to hold; transmits when your level crosses a threshold, with 700 ms hangover + 60 ms pre-roll so words aren't clipped. The threshold **rides the measured background**, so one setting works both in a quiet room and at highway speed instead of holding the channel open on wind noise.
...
- **Adaptive VOX floor:** the threshold rides the measured background, so the same setting works in a quiet room and at highway speed. Frames loud enough to be speech are excluded from the estimate, or a long sentence would drag the floor up behind it and gate the speaker mid-word. VOX off (0 %) still means nothing is ever held back — which also means the adaptation only runs once the gate is armed at all, and arming it is what **Riding mode** is for.
